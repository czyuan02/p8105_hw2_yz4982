...
```
A comparison of the ZIP-code crosswalk and Zillow indicates **`r length(zips_only_in_zipfile)`** ZIP codes present in the crosswalk but absent from the rental panel. The missing ZIPs generally correspond to non-residential or low-inventory areas—PO-box ZIPs, airports, discontinued new codes, or ZIPs with insufficient listings. Examples include 11371 (LaGuardia Airport), 11430 (JFK Airport), and Midtown 101xx ZIPs like 10118.
...
top10_drop <- final_df |>
  ...
  select(zip, borough, neighborhood, ymu, zori) |>
  distinct(zip, ymu, .keep_all = TRUE) |>
  pivot_wider(
    names_from  = ymu,
    values_from = zori,
    names_prefix = "zori_"
  ) |>
  # Keep the ZIP for only two months without missing a beat
  filter(!is.na(zori_2020_01), !is.na(zori_2021_01)) |>
  # Calculation of decline
  mutate(
    drop_amt = zori_2020_01 - zori_2021_01,
    drop_pct = 100 * drop_amt / zori_2020_01
  ) |>
  arrange(desc(drop_amt)) |>
  slice_head(n = 10) |>
  mutate(
    zori_2020_01 = round(zori_2020_01, 0),
    zori_2021_01 = round(zori_2021_01, 0),
    drop_amt     = round(drop_amt, 0),
    drop_pct     = sprintf("%.1f%%", drop_pct)
  )

print(top10_drop)
knitr::kable(top10_drop, caption = "Top 10 ZIPs: Largest Rent Drops, Jan 2020 → Jan 2021")
```
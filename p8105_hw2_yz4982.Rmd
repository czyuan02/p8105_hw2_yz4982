...
# Check
cat("Total Row: ", nrow(final_df), "\n", sep = "")
cat("Different ZIP numbers: ", length(unique(final_df$zip)), "\n", sep = "")
cat("Time range: ", as.character(min(final_df$month, na.rm = TRUE)),
    " ~ ", as.character(max(final_df$month, na.rm = TRUE)), "\n", sep = "")
cat("缺失比例 - borough: ", round(mean(is.na(final_df$borough))*100, 1), "%\n", sep = "")
cat("缺失比例 - county:  ", round(mean(is.na(final_df$county_clean))*100, 1), "%\n", sep = "")
cat("缺失比例 - neigh.:  ", round(mean(is.na(final_df$neighborhood))*100, 1), "%\n", sep = "")
cat("缺失比例 - zori:    ", round(mean(is.na(final_df$zori))*100, 1), "%\n", sep = "")
print(utils::head(final_df, 10))

# Output
readr::write_csv(final_df, "nyc_zori_2015_2024_clean.csv")
cat("Already download nyc_zori_2015_2024_clean.csv\n")
```